...
/*   Updated: 2024/12/07 10:36:28 by ktiomico         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	**process_map(const char *file, t_game *game)
{
	...
	if (validate_map(map) || validate_reachability(map, rows, cols) == CHECK_KO)
	{
		ft_printf("\033[31mError: Invalid map structure.\033[0m\n");
		cleanup_game(game);
		return (NULL);
	}
	ft_printf("\033[32m[✔] Map is playable.\033[0m\n");
	return (map);
}
